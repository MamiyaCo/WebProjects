...
$(document).ready(function(){
	...
	if ($('.promoShipping').length>0 )
	{
		$(window).load(function(){$('.promoShipping').addClass('show');});
	}
	...
	$("#searchInp").keyup(function(event){
		if(event.keyCode == 13){		
			$('#menu.opn').removeClass('opn');
			$('.menusmall.opn').removeClass('opn');
			location.href="/search-results#stq="+$(this).val();
		}
	});

	$(document).on("click",".fq>.ftitle,.fq>.fdownload.open",function(){
		$(this).closest(".fq").find(".fdetails").is(":visible")?$(".fdetails").hide():($(".fdetails").hide(),$(this).closest(".fq").find(".fdetails").show());
	});
	...
	if ($('#topLinks').length>0) {
		h=180;
		if ($('.submenuadmin').length>0) h=480;
	...
	if (window.location.hash.length>=0 && $(window).width()<1000) 
	{
		if ($(window.location.hash).length>0)
		{
			$(window.location.hash).prev("h2.mobiletab").click();
			$(window).load(function(){
				$('body,html').animate({
						scrollTop: $(window.location.hash).prev("h2.mobiletab").offset().top
				}, 1);
			});
		}
	}
	...
	if ($('.product_buy select.form').length>0)
	{
		...
		{		
			...
			else
			{
				...
				$('#MailingList iframe').css('height',500);
			}
		}
		if (xget('Redirecting') && xget('MailingList2')) 
		{		
			if ($(window).width()<=450) 
			{
				xget('MailingList2').style.width='100%';			
				xget('MailingList2').style.left='0';	
			}
			else
			{
				xget('MailingList2').style.width='450px';			
				xget('MailingList2').style.left=(parseInt(getViewportWidth()/2)-225)+'px';	
			}
		}
...
function UPTvalidateform(thisform)
{
	...
	{	
		...
		thisform.method='post'; thisform.action='http://app.maropost.com/accounts/551/forms/2480/subscribe/67909c04560abc4ac39169dcf590dc97d4b57c09'; 
		thisform.submit();		
		return false;
	}
	else
...
function showcart(obj)
{
	...
	$.ajax({
		...
		success: function(data) {	
			...
			$.ajax({
				url: '/shopping-cart?ie_fix='+ie_fix,
				success: function(data) {	
					...
					$('.cartsmall .btn_red2').click(function(){closecart();return false;});
				}
			});
		}
	});	
